...
class TripletNet(pl.LightningModule):
    ...
    def __init__(self, verbose=True, **config_kwargs):
        ...
        self.dataset, self.train_triplets, self.valid_triplets = self.get_data()
    ...
    def get_data(self):
        ...
        train_img_idx = np.random.choice(total_idx, int(len(total_idx)*0.8), replace=False)
        ...
        # valid_dataset = torch.utils.data.TensorDataset(torch.from_numpy(valid_data), torch.from_numpy(valid_label))
        ...
        return dataset, np.array(train_triplets), np.array(valid_triplets)
