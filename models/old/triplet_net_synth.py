...
class TripletNet(pl.LightningModule):
    ...
    def __init__(self, verbose=False, **config_kwargs):
        ...
        # total_combs = torch.combinations(torch.arange(0, len(self.dataset)).int(), r=3)
        # subset = np.random.choice(np.arange(0,len(total_combs)), 5600)
        # self.triplets = total_combs[subset]
        ...
        pickle.dump((self.train_triplets,self.val_triplets),open("triplets.pkl","wb"))
        # self.train_triplets = self.triplets
        # self.val_triplets = torch.combinations(torch.arange(0, len(self.valid_dataset)).int(), r=3)
        print(len(self.train_idx))
        print(len(self.val_idx))
        # print(self.subset_idx)
        
        self.train_pairwise_distance = torch.Tensor(pickle.load(open(self.hparams.train_pairwise_distance, "rb")), device=self.device)
        self.pairwise_distance = self.train_pairwise_distance[self.subset_idx]
    ...
    def get_train_dataset(self):
        ...
        # subset = np.random.choice(np.arange(len(dataset)), 20)
        # self.subset_idx = subset
        # print(subset)
        self.subset_idx = [102,92,14,106,71,20,102,121,74,87,116,99,103,151,130,149,52,1,87,157]
        dataset = [dataset[i][0].numpy() for i in self.subset_idx]
        dataset = torch.utils.data.TensorDataset(torch.from_numpy(np.array(dataset)))
        return dataset
...
def main():
    ...
    pickle.dump(embeds, open("xd.pkl", "wb"))

    def euc_dist(x, y): return np.sqrt(np.dot(x, x) - 2 * np.dot(x, y) + np.dot(y, y))
    train_align = []
    train_dist = model.pairwise_distance
    ...
    trainer, model = train(model, args)
