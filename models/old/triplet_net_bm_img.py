...
class RESN(pl.LightningModule):
    ...
    def __init__(self, verbose=True, **config_kwargs):
        ...
        self.train_dataset, self.valid_dataset = self.get_data()
    ...
    def forward(self, combs):
        ...
        triplets = (embeds[triplet_idx[:,0]], embeds[triplet_idx[:,1]], embeds[triplet_idx[:,2]])
        return triplets
    ...
    def get_data(self):
        train_dataset = torchvision.datasets.ImageFolder(self.hparams.train_dir, transform=self.parse_augmentation())
        ...
        valid_dataset = torchvision.datasets.ImageFolder(self.hparams.valid_dir, transform=val_transform)
        
        train_dataset = torch.tensor(np.array([data[0].numpy() for data in train_dataset])).cuda()
        valid_dataset = torch.tensor(np.array([data[0].numpy() for data in valid_dataset])).cuda()

        return train_dataset, valid_dataset

    def train_dataloader(self):
        train_triplets = torch.combinations(torch.range(0, len(self.train_dataset)-1).long(), r=3)
        dataset = torch.utils.data.TensorDataset(train_triplets)
    ...
    def val_dataloader(self):
        valid_triplets = torch.combinations(torch.range(0, len(self.valid_dataset)-1).long(), r=3)
        dataset = torch.utils.data.TensorDataset(valid_triplets)
        dataloader = torch.utils.data.DataLoader(
            dataset, 
            batch_size=self.hparams.train_batch_size, 
            num_workers=self.hparams.dataloader_num_workers, 
