...
class TripletNet(pl.LightningModule):
    ...
    def __init__(self, verbose=False, **config_kwargs):
        ...
        total_combs = torch.combinations(torch.arange(0, len(self.train_dataset)).int(), r=3)
        ...
        print(len(self.train_triplets))
        print(len(self.val_triplets))
    ...
    def get_valid_dataset(self):
        ...
        # subset = np.random.choice(np.arange(len(dataset)), len(dataset)//2)
        # dataset = [dataset[i][0].numpy() for i in subset]
        # 
