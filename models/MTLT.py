...
class MTLT(pl.LightningModule):
    ...
    def sample_xs_ys(self, x_idx, dataset):
        xs, ys = [], []
        for idx in x_idx:
            x, y = dataset[idx]
    ...
    def training_step(self, batch, batch_idx):
        ...
        xs, ys = self.sample_xs_ys(batch_idx, self.train_dataset)
    ...
    def test_step(self, batch, batch_idx):
        ...
        if knn_acc and ds_acc:
            self.log('test_knn_acc', knn_acc)
            self.log('test_decision_support', ds_acc)

    def eval_knn_ds(self, test_ds, train_ds, syn_x_train=None, syn_x_test=None):
        x_train, y_train = self.sample_xs_ys(range(len(train_ds)), train_ds)
        x_test, y_test = self.sample_xs_ys(range(len(test_ds)), test_ds)
        ...
        if self.hparams.syn:
            ...
            syn_z_train = syn_trans(syn_x_train, w1=w1, w2=w2)
            syn_z_test = syn_trans(syn_x_test, w1=w1, w2=w2)
            ...
            ds_acc = eval_nn_mat(syn_dst, nn_mat, y_test, y_train)
