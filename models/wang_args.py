...
class WangClassifier(pl.LightningModule):
    ...
    def __init__(self, verbose=False, **config_kwargs):
        ...
        out_size = 7 if self.hparams.pooling else 224
    ...
    def forward(self, x):
        x = self.embed(x)
        if self.embed_dim > 1:
            x = x.view(x.shape[0], self.num_sequences + 1, -1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, m in enumerate(ms):
            ...
            self.log(f'seq_{sid}_acc', m['acc'])
            self.log(f'seq_{sid}_auc', m['auc'])
            self.log(f'seq_{sid}_sensitivity', m['tpr'])
            self.log(f'seq_{sid}_specificity', m['tnr'])
            self.log(f'seq_{sid}_precision', m['ppv'])
            self.log(f'seq_{sid}_f1', m['f1'])
            self.log(f'seq_{sid}_ap', m['ap'])
            self.log(f'seq_{sid}_auprc', m['auprc'])
